...
// to say "数据不足" when a metric is weak. If the model is unavailable or returns junk, a
// deterministic fallback narrates the same numbers, so the card is always grounded and demo-safe.
...
export function buildInsightsBrief(insights: MerchantInsights): string {
  const s = insights.snapshot;
  const lines: string[] = [
    `本周快照（近${s.rangeDays}天）：试戴 ${s.tryOns}，预订 ${s.bookings}，搜索 ${s.searches}，活跃顾客 ${s.activeCustomers}。`,
  ];

  const trends = insights.demandTrends.slice(0, 5).map((t) => `${t.label} 本周${t.current}/上周${t.previous}（${t.direction === 'up' ? '↑' : t.direction === 'down' ? '↓' : '→'}）`);
...
export function fallbackSummary(insights: MerchantInsights): AISummary {
  const s = insights.snapshot;
  const hasData = s.tryOns + s.bookings + s.searches > 0 || insights.demandTrends.length > 0;
  if (!hasData) {
    return { headline: '数据不足，暂无足够行为数据生成洞察。', insights: [], actions: [], source: 'fallback' };
  }
...
/** Reject after `ms` so a slow/hanging model never blocks the dashboard's AI card. */
...
export async function summarizeInsights(
  ...
  if (!apiKey) return fallbackSummary(insights);
  ...
  const payload: OpenRouterPayload = {
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `以下是本店已计算好的经营指标，请据此生成 headline、最多3条 insights、最多2条 actions：\n\n${buildInsightsBrief(insights)}`,
      },
    ],
  };
  ...
  try {
    ...
    return parsed ? { ...parsed, source: 'ai' } : fallbackSummary(insights);
  } catch {

...
import MerchantSchedulePage from './page';

// The full appointment calendar moved here from /merchant/calendar when that route became the 今日
// agent-ops home (audit 2026-07-06). These are the calendar-behavior tests, repointed to the new route.

vi.mock('next/navigation', () => ({
  usePathname: () => '/merchant/calendar/schedule',
  useRouter: () => ({
    push: vi.fn(),
    replace: vi.fn(),
    prefetch: vi.fn(),
...
describe('MerchantSchedulePage (full calendar)', () => {
  ...
  beforeEach(() => {
    ...
    // The calendar reads bookings from the repository-backed booking service; reset the in-memory
    // bundle so each test starts from the derived demo seed (booking-001..004).

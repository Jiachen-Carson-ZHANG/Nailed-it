...
export function StyleWaterfallGrid({ hideHeading = false, styles }: StyleWaterfallGridProps) {
  if (styles.length === 0) {
    return (
      <section aria-labelledby="trending-style-grid-title" className="discovery-section">
        {hideHeading ? <h2 className="sr-only" id="trending-style-grid-title">Filtered results</h2> : null}
        {hideHeading ? null : (

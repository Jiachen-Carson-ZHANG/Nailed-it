...
| — | **P4e cleanup** (remove the localStorage path + retire the flat tables) | **partial**: the booking/message `operations-store` localStorage path is removed; old flat tables remain. The newly introduced glossary-settings localStorage path is a separate conflicting pricing fork and must move onto `merchant_pricing`, not become a replacement persistence path. |

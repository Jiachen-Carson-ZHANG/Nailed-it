...
| — | **P6.5 merchant style library + media foundation** (`media_asset` + `merchant_style` + relational `merchant_style_item`; Supabase Storage originals/published buckets; merchant review/publish; customer published feed) | **live (migrations `0009`–`0014` applied)**: separates physical media ownership from the style publication lifecycle. The catalog breakdown is the relational `merchant_style_item` table; publish derives price/duration from it via quoteService. Merchant Me/library and customer discovery/detail are wired through scoped actions; customer reads are published-only. Remaining: the 34 live styles still need `npm run configure:styles` to populate their breakdowns (default until per-image config lands). See `docs/plans/2026-06-06-p6-5-merchant-style-library-design.md`. |
